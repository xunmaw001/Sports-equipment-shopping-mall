...
<body>
						...
						<div class="widget-area-2 lochana-box-shadow">
							...
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>保修期</label>
										<input name="baoxiuqi" id="baoxiuqi" name="baoxiuqi" class="form-control" placeholder="保修期">
									</div>
									<div class="form-group col-md-6">
										<label>上架日期</label>
									...
									<div class="form-group col-md-6">
										...
										<input name="alllimittimes" id="alllimittimes" name="alllimittimes" class="form-control" placeholder="库存">
											...
											<div class="form-group  col-md-12" >
												<label>器材介绍</label>
												<div><div class="upload"><input id="qicaijieshaoupload" name="file" type="file">选择文件</div>
												<script id="qicaijieshaoEditor" type="text/plain"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var qicaijieshaoUe = UE.getEditor('qicaijieshaoEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="qicaijieshao" id="qicaijieshao-input">
												</div>
											</div>	
	...
	<script>
		...
		function upload(){
			...
			$('#qicaijieshaoupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('qicaijieshaoEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.qicaijieshao != null && ruleForm.qicaijieshao != 'null' && ruleForm.qicaijieshao != ''){
				var qicaijieshaoMes = '' + ruleForm.qicaijieshao;
				var qicaijieshaoUeditor = UE.getEditor('qicaijieshaoEditor');
				qicaijieshaoUeditor.ready(function() {
					qicaijieshaoUeditor.setContent(qicaijieshaoMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('qicaijieshaoEditor').hasContents()){
				$('#qicaijieshao-input').attr('value',UE.getEditor('qicaijieshaoEditor').getContent());
			}
		}
		...
		function dataBind(){
			...
			$("#baoxiuqi").val(ruleForm.baoxiuqi);	
			$("#shangjiariqi-input").val(ruleForm.shangjiariqi);
			$("#qicaijieshao").val(ruleForm.qicaijieshao);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.baoxiuqi != null && res.data.baoxiuqi != ''){

						$("#baoxiuqi").val(res.data.baoxiuqi);
						$("#baoxiuqi").attr('readonly','readonly');
					}
					...
					if(res.data != null && res.data != '' && res.data.qicaijieshao != null && res.data.qicaijieshao != ''){

						$("#qicaijieshao").val(res.data.qicaijieshao);
                        var qicaijieshaoMes = '' + res.data.qicaijieshao;
                        var qicaijieshaoUeditor = UE.getEditor('qicaijieshaoEditor');
                        qicaijieshaoUeditor.ready(function() {
                            qicaijieshaoUeditor.setContent(qicaijieshaoMes);
                        });
